...
public class KafkaMessagePublisherImpl implements KafkaMessagePublisher{
    ...
    public void publishMessage(String message) {
        log.info("Topic Name : {}", publishTopic);
    ...
    public void publishEmployee(Employee employee) {
        log.info("Topic Name : {}", employeeTopic.name());
        CompletableFuture<SendResult<String, Object>> future = kafkaTemplate.send(employeeTopic.name(), employee);
        future.whenComplete((result, ex) -> {
            log.info("publish the message in Topic Partition : {}", result.getRecordMetadata().partition());
            if(ex == null) {
                log.info("Sent Message=[" + employee.toString() + "] with Offset=[" + result.getRecordMetadata().offset() + "]");
            } else {
                log.error("Unable to send Message=[" + employee.toString() + "] due to :" + ex.getMessage());
            }
        });
    }
}
